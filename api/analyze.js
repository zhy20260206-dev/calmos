...
export default async function handler(req) {
  ...
  try {
    ...
    if (!content) {
      console.error('Empty response from DeepSeek:', JSON.stringify(data));
      return new Response(
    ...
    const required = ['primary_cause', 'summary', 'evidence', 'recommended_modules', 'module_details', 'closing_message'];
    ...
    if (!Array.isArray(result.module_details) || result.module_details.length < 2) {
